    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pinjams.update", [$pinjam->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime-picker {{ $errors->has('time_start') ? 'is-invalid' : '' }}" type="text" name="time_start" id="time_start" value="{{ old('time_start', $pinjam->time_start) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime-picker {{ $errors->has('time_end') ? 'is-invalid' : '' }}" type="text" name="time_end" id="time_end" value="{{ old('time_end', $pinjam->time_end) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_hp') ? 'is-invalid' : '' }}" type="text" name="no_hp" id="no_hp" value="{{ old('no_hp', $pinjam->no_hp) }}" required>
                @if($errors->has('no_hp'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('penggunaan') ? 'is-invalid' : '' }}" name="penggunaan" id="penggunaan" required>{{ old('penggunaan', $pinjam->penggunaan) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('unit_pengguna') ? 'is-invalid' : '' }}" name="unit_pengguna" id="unit_pengguna">
                    ...
                    @foreach(App\Models\Pinjam::UNIT_PENGGUNA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('unit_pengguna', $pinjam->unit_pengguna) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
